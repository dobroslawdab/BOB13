...
class SolanaScanner {
  ...
  /**
   * ENHANCED: Zapisuje historię skanowania z lepszym debugowaniem
   */
  async storeScanHistory(scanData) {
    try {
      console.log('📝 === STORING SCAN HISTORY ===');
      console.log('Scan data to store:', JSON.stringify(scanData, null, 2));
      ...
      // Próba zapisu z pełnym logowaniem
      console.log('💾 Attempting to insert scan history...');
      const { data, error } = await supabase
        .from('scan_history')
        .insert([scanData])
        .select(); // Dodajemy .select() żeby zobaczyć co zostało zapisane
  ...
  /**
   * ENHANCED: Pobiera historię skanów z lepszym debugowaniem
   */
  async getScanHistory(limit = 10) {
    try {
      ...
      console.log(`✅ Retrieved ${data?.length || 0} scan history records`);
  ...
  async scanPrice(scanType = 'automatic') {
    ...
    try {
      ...
      // 7. ENHANCED: Zapisz historię skanowania (SUKCES)
      ...
      // 8. ENHANCED: Zapisz historię skanowania (BŁĄD)

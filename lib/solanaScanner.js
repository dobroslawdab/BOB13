 ...
 * UPDATED: Uses existing scan_history table structure with JSON details
 */
...
class SolanaScanner {
  ...
  /**
   * UPDATED: Zapisuje historię skanowania używając istniejącej struktury tabeli
   */
  async storeScanHistory(scanData) {
    try {
      console.log('📝 === STORING SCAN HISTORY ===');
      console.log('Original scan data:', JSON.stringify(scanData, null, 2));
      
      const { supabase } = await import('./supabase.js');
      console.log('✅ Supabase client imported successfully');

      // Mapowanie danych do istniejącej struktury tabeli
      const mappedData = {
        ...
        details: {
          // Wszystkie szczegóły przechowujemy w JSON
      ...
      console.log('Mapped data for insert:', JSON.stringify(mappedData, null, 2));

      // Test połączenia z tabelą
      ...
      // Próba zapisu
      console.log('💾 Attempting to insert scan history...');
      const { data, error } = await supabase
        .from('scan_history')
        .insert([mappedData])
        .select();
      ...
      if (error) {
        console.error('❌ Error storing scan history:');
      ...
      console.log('=== SCAN HISTORY STORED ===');
      
    } catch (error) {
      console.error('❌ Exception in storeScanHistory:');
      console.error('Error message:', error.message);
  ...
  /**
   * UPDATED: Pobiera historię skanów z nowej struktury
   */
  async getScanHistory(limit = 10) {
    try {
      console.log('🔍 === FETCHING SCAN HISTORY ===');
      console.log(`Fetching last ${limit} scan history records...`);
      ...
      // Mapowanie z nowej struktury do formatu oczekiwanego przez frontend
      ...
      console.log(`✅ Retrieved and mapped ${mappedData.length} scan history records`);
      console.log('Sample mapped record:', mappedData[0] || 'No records');
      console.log('=== SCAN HISTORY FETCHED ===');
  ...
  /**
   * ENHANCED: Główna funkcja skanowania z lepszym logowaniem historii
   */
  async scanPrice(scanType = 'automatic') {
    ...
    try {
      ...
      // 7. UPDATED: Zapisz historię skanowania (SUKCES)
      console.log('📝 Preparing scan history data...');
      ...
      console.log('📝 Calling storeScanHistory...');
      await this.storeScanHistory(scanHistoryData);
      ...
      // 8. UPDATED: Zapisz historię skanowania (BŁĄD)
      console.log('📝 Preparing ERROR scan history data...');
      const errorScanHistoryData = {
        scan_type: scanType,
        timestamp: scanTimestamp.toISOString(),
        status: 'error',
      ...
      console.log('📝 Calling storeScanHistory for ERROR...');
      await this.storeScanHistory(errorScanHistoryData);
